...
sudo -u "$LOOP_USER" bash << EOF
    ...
    # Fix errors with submodule references due to renamed modules
    # 1. Stage .gitmodules if it has modifications (required for git rm --cached to work if index is dirty)
    if ! git diff --quiet .gitmodules; then
         echo "Staging .gitmodules changes to allow submodule operations..."
         git add .gitmodules
    fi

    # 2. Remove phantom submodules that exist in index but not in .gitmodules
    # "_NIX-SYSOP-AI-AGENT-by-Aris-Arjuna-Noorsanto" is the old name of "ai/nemodian_base"
    # "ai/workspace" appears to be an untracked nested repo or artifact
    for phantom in "_NIX-SYSOP-AI-AGENT-by-Aris-Arjuna-Noorsanto" "ai/workspace"; do
        if git ls-files --stage | grep -q "\$phantom"; then
             echo "Removing phantom submodule from index: \$phantom"
             git rm --cached "\$phantom" || true
        fi
    done
